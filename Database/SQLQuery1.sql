...
BEGIN
    ...
    BEGIN
        ...
        -- 1. CLEAN UP GAME DATA (If the user is a child)
        DELETE FROM tblGameSession WHERE UserId = @UserId;
        DELETE FROM tblMemoryScoreSummary WHERE UserId = @UserId;

        -- 2. HANDLE DOCTOR RELATIONSHIPS
        DECLARE @DoctorIdToDelete INT;
        SELECT @DoctorIdToDelete = DoctorId FROM tblDoctorProfile WHERE UserId = @UserId;

        -- If this user is a doctor, set the DoctorId to NULL for all their patients
        IF @DoctorIdToDelete IS NOT NULL
        BEGIN
            UPDATE tblChildProfile 
            SET DoctorId = NULL 
            WHERE DoctorId = @DoctorIdToDelete;
        END
